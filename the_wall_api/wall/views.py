...
from the_wall_api.wall.serializers import (
    DailyIceSerializer,
    ProfileOverviewSerializer,
    GlobalOverviewSerializer
)
...
class WallViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def profile_overview(self, request, profile_number=None, day_number=None):
        profile = self.get_object()
        serializer = ProfileOverviewSerializer(profile, context={'day_number': day_number})
        return Response(serializer.data)

    @method_decorator(cache_page(60 * 60))
    @method_decorator(vary_on_cookie)
    @action(detail=False, methods=['get'], url_path=r'overview(?:/(?P<day_number>\d+))?')
    def overview(self, request, day_number=None):
        # Manually building the object for a non-model serializer
        data = {
            "day": str(day_number) if day_number else "None",
            "cost": None  # The serializer method field will overwrite this
        }
        serializer = GlobalOverviewSerializer(data, context={'day_number': day_number})
        return Response(serializer.data)
